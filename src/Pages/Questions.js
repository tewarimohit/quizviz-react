        ...
        <div>
            <header>
                <div className='header'>
                    <h2 className='hp-font'>{questions[index].category}</h2>
                    <Link to='/'>
                        <button className='header-btn'>HOME</button>
                    </Link>
                    <h4 className='hp-font text-color margin-bottom'>{questions[index].quote}</h4>
            ...
            <section className='question-section container'>
                <div className='question-part'>
                    <h4 className='hp-font'>{questions[index].question}</h4>
                    {questions[index].options.map((each, i) => (
                        <button key={`button-key${i}`} type='button' className={`btns btn-grey ${Boolean(selectedOptions[index]) && (each === questions[index].correctAns ? 'btn-green' : 'btn-red')}`} name='button' onClick={() => optionClickHandler(each, index)}>
                            {each}
                        </button>
                    ))}
                </div>
            ...
            <div className='footer'>
                <div className='footer-container'>
                    <div>
                        <button className='footer-btn' onClick={onClickPrevHandler}>PREVIOUS</button>
                    </div>
                    <div className='margin-auto'>
                        <button
                            className='footer-btn'
                            onClick={questions[index].questionId === 5 ? onClickScoreHandler : onClickNextHandler}
                        >
